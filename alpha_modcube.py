...
from jax_fem.solver import solver, get_flatten_fn
from jax_fem.utils import save_sol
from jax_fem.generate_mesh import box_mesh, get_meshio_cell_type, Mesh,read_in_mesh,cells_out
...
def get_shape_grads_ref(ele_type):
    ...
    shape_grads_ref = np.transpose(vals_and_grads[1:, :, :, 0], axes=(1, 2, 0))
...
def get_shape_grads_physical(problem):
    ...
    jacobian_dx_deta = np.sum(physical_coos[:, None, :, :, None] * shape_grads_ref[None, :, :, None, :], axis=2, keepdims=True)

    jacobian_deta_dx = np.linalg.inv(jacobian_dx_deta) # (num_cells, num_points, 1, dim, dim)
...
if __name__ == "__main__":
    # parser = argparse.ArgumentParser(description="Attempt forward pass.")
    # parser.add_argument(
    #     "--meshpath",
    #     default=os.path.join("mesh", "toy2d.msh")
    # )
    # parser.add_argument(
    #     "--outputpath",
    #     default=os.path.join("soln", "u.vtu")
    # )
    # args = parser.parse_args()

    # main(args.meshpath, args.outputpath)
    main("","")