...
def xcell_displacement(point, load_factor=1):
    ...
    ind = np.where(np.linalg.norm(init_pos-point,axis=1) < 10**-2,size=1)[0][0]
    # print("ind",np.where(np.linalg.norm(init_pos-point,axis=1) < 10**-2,size=1))
...
def zcell_displacement(point, load_factor=1):
    ...
    ind = np.where(np.linalg.norm(init_pos-point,axis=1) < 10**-2,size=1)[0][0]
    #ind = np.argwhere(np.linalg.norm(init_pos-point,axis=1) < tol)
    return disp[ind][2] * load_factor


def problem():
    # print("aa",xcell_displacement(np.array([100,300,300])))
    # print("bb", xcell_displacement(np.array([8.087059020996093750e+01,8.361865997314453125e+01, 1.628457450866699219e+01])))
    # print(8.087059020996093750e+01+xcell_displacement(np.array([8.087059020996093750e+01,8.361865997314453125e+01, 1.628457450866699219e+01])))
    ...
    def cell_surface(point):
        return np.all(np.isclose(point, np.array(pdata),atol=10**-2),axis=0)
