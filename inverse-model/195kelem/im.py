...
def main():
    ...
    a_quad = ref_problem.fes[0].convert_from_dof_to_quad_a(alpha_mat.reshape(ref_problem.fes[0].points.shape[0],1))[:, :, 0]
    ...
    result = minimize(obj_and_grad, a_in, method='L-BFGS-B', jac=True, bounds = [(.5, 10)]*a_0.size, tol = 10**-8,options = {"maxiter":2000,"gtol":10**-8,"disp": True,"iprint":100}, )
