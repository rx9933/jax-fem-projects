...
stretch_factor = 3.83 # 3.87 gives NaNs, for 10 steps
a = stretch_factor - 1
...
def apply_load_steps(problem, initial_sol, num_steps):
    ...
    for step in np.arange(0, 1, 1 / num_steps):
...
#         print("127")
#         print("STEP",step)
#         problem.dirichlet_bc_info[0][2][3:] = [lambda point: surface_x(point,load_factor),lambda point: surface_y(point,load_factor),lambda point: surface_z(point,load_factor)]
#         sol = solver(problem, use_petsc=True, initial_guess=sol) # before, res l_2 = 192.72610161188817
...
def problem():
    ...
    for i in range(num_cells):
        for j in range(num_points):
            global_pointind = cells[i,j]
            point_val = points[global_pointind] # 1d, len 3 array of a single point (current location)
            stress_tensor = stress_tensor.at[i,j,:,:].set(first_PK_stress(u_grads[i,j],point_val))
            j_mat = j_mat.at[i,j].set(get_j(get_f(u_grads[i,j])))
            alpha_mat = alpha_mat.at[i,j].set(get_alpha(point_val))
        if i%100 == 0:
            print("i:",i)
