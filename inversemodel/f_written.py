...
def get_alpha(x0):
    ...
    x0 = np.array(x0, dtype=onp.float32)
    # print("X0,", x0)

    ind = np.where(np.linalg.norm(points-x0) < tol, 1, 0)
    
    ind = np.array(ind, dtype=onp.int32)
    # print("ind",ind)
    # print("alpha_f", type(alpha_f))
    # jax.debug.print("IND: {}", ind)
    # i = 1
    # i = np.nonzero(ind,size=1)#(ind==np.array([1,1,1]))
    # print(np.take(alpha_f, i))
    # print("points",points)
    # print("x0",x0)
    # print("ind", ind)
    # print("io", i[0])
...
def main(alpha,s):
    ...
    # alpha =onp.asarray(alpha.astype(float))
    # jax.debug.print("debug: {}", alpha)
    # np.save("alpha.npy", alpha)
    # save current version of alpha, as passed in through i.py
    t = np.zeros(l)
    for i in range(l):
        start_time = time.time()
        print("CALLED PROBLEM")
        problem(s)
