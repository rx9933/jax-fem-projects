...
def xcell_displacement(point, load_factor=1):
    ...
    # print(np.linalg.norm(init_pos[:,:2]-point[:2],axis=1).shape)
    ind = np.argwhere(np.linalg.norm(init_pos[:,:2]-point[:2],axis=1) < tol,size=1)
    # print("ind",ind)
    # for r in ind:
    #     if r[0]!=0:
    #         print("YYY")
...
def problem():
    ...
    problem = HyperElasticity(mesh, vec=3, dim=3, ele_type=ele_type, dirichlet_bc_info=dirichlet_bc_info)
    print("TEST", cell_surface(np.array([67.1986, 227.034, 62.3789])))
    print("TEST", cell_surface(np.array([67.1986, 100.111, 216.744])))
