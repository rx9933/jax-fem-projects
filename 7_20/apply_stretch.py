...
disp = np.asarray(onp.load("cell_vertices_stretched.npy")) - init_pos

print(onp.load("cell_vertices_stretched.npy")[0])
tol = 10**-9

def zero_dirichlet_val(point, load_factor=1):
    return 0.0*load_factor
...
def problem():
    ...
    sol = apply_load_steps(problem, 5)
    ...
    vtk_path = os.path.join(data_dir, f'vtk/apply_stretch.vtu')
