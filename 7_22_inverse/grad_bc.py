...
def main():
    ...
    def save_sol_all(sol):
        #print("DIFF", np.linalg.norm(sol[0]-sol_0[0]))
        ...
        #print("CCC",sol[0][0])
        print("ddd", problem.fes[0].cells[0])
        C = get_c(f_vals)
    ...
    def test_fn(sol_list):
        ...
        if flag_1 == False:
            ...
            #print(f'test = {np.linalg.norm(C_c-C_0):.7f}')
    ...
    problem.set_params(params)
    fwd_pred = ad_wrapper(problem)
    curr_sol = solver(problem, use_petsc = True)[0]
    print("DIFFERENCE",np.linalg.norm(curr_sol[0]-sol_0[0]))
