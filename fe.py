...
class FiniteElement:
    ...
    def convert_from_dof_to_quad_a(self, sol):
        """Obtain quad values from nodal solution

        Parameters
        ----------
        sol : np.DeviceArray
            (num_total_nodes, val) # val = 1 scalar number

        Returns
        -------
        u : np.DeviceArray
            (num_cells, num_quads, val)
        """
        # (num_total_nodes, val) -> (num_cells, num_nodes, val)
        cells_sol = sol[self.cells]
        # (num_cells, 1, num_nodes) * (1, num_quads, num_nodes, 1) -> (num_cells, num_quads, num_nodes, val) -> (num_cells, num_quads, val)
        u = np.sum(cells_sol[:, None, :, :] * self.shape_vals[None, :, :, None], axis=2)
        return u
    
    def convert_from_dof_to_quad_C(self, sol):
        """Obtain quad values from nodal solution

        Parameters
        ----------
        sol : np.DeviceArray
            (num_total_nodes, vec, vec)
        ...
        cells_sol = sol[self.cells]
        # (num_cells, 1, num_nodes, val) * (1, num_quads, num_nodes, 1 ,1) -> (num_cells, num_quads, num_nodes, vec, vec) -> (num_cells, num_quads, vec, vec)
        u = np.sum(cells_sol[:, None, :, :] * self.shape_vals[None, :, :, None, None], axis=2)
        return u
